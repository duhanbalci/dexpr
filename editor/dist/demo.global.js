...
(() => {
  ...
  function inferVariableTypes(context, knownVars) {
    const types2 = new Map(knownVars);
    ...
    tree.iterate({
      enter(node) {
        ...
        const varName = doc2.sliceString(varNode.from, varNode.to);
        const assignOp = varNode.nextSibling;
        if (!assignOp) return;
        const exprNode = assignOp.nextSibling;
        if (!exprNode) return;
        const exprType = inferExprType(exprNode, doc2, types2);
        if (exprType) types2.set(varName, exprType);
      }
    });
    return types2;
  }
  function inferExprType(node, doc2, knownTypes) {
    switch (node.name) {
      case "String":
        return "String";
      case "Number":
      ...
      case "BooleanLiteral":
        return "Boolean";
      case "VariableName": {
        const name2 = doc2.sliceString(node.from, node.to);
        return knownTypes.get(name2) ?? null;
      }
      case "MethodCall": {
        const propNode = findChild2(node, "PropertyName");
        if (!propNode) return null;
        const method = doc2.sliceString(propNode.from, propNode.to);
        return inferMethodReturnType(method);
      }
      case "BinaryExpression": {
        const first = node.firstChild;
        if (first) {
          const t2 = inferExprType(first, doc2, knownTypes);
          if (t2 === "String") return "String";
          if (t2 === "Number") return "Number";
        }
        return null;
      }
      case "PropertyAccess": {
        ...
        if (objNode.name === "VariableName") {
          const varName = doc2.sliceString(objNode.from, objNode.to);
          const fieldName = doc2.sliceString(propNode.from, propNode.to);
  ...
  function findChild2(node, name2) {
    ...
    function resolveDotPath(context, dotPos, varTypes) {
      ...
      while (true) {
        ...
        if (nodeAtPos.name === "PropertyName") {
          path.unshift(doc2.sliceString(nodeAtPos.from, nodeAtPos.to));
          const dotCharPos = nodeAtPos.from - 1;
          if (dotCharPos >= 0 && doc2.sliceString(dotCharPos, dotCharPos + 1) === ".") {
            pos = dotCharPos;
            continue;
          }
          break;
  ...
  // src/index.ts
  function dexpr(config2) {
    const extensions = [
      new LanguageSupport(dexprLanguage),
      dexprCompletion(config2)
    ];
    if (config2.highlighting !== false) {
  ...
  var languageInfo = {
    ...
    variables: [
      ...
      // → from_json auto-detects field types from the JSON values
      {
        name: "customer",
        type: "Object",
        doc: "Customer \u2014 from JSON via Value::from_json()",
        fields: [
      ...
      // info.add_value("order", &Value::from_json(r#"{"amount":1500,...}"#)?, None)
      {
        name: "order",
        type: "Object",
        doc: "Order \u2014 from JSON via Value::from_json()",
        fields: [
